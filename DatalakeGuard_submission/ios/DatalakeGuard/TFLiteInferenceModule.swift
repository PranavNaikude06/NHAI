...
class TFLiteInference: NSObject, RCTBridgeModule {
  ...
  @objc(getModelDiagnostics:rejecter:)
  func getModelDiagnostics(_ resolve: @escaping RCTPromiseResolveBlock, rejecter reject: @escaping RCTPromiseRejectBlock) {
    let models: [(String, String, Bool)] = [
      ("BlazeFace (Detection)", "blazeface", blazeFaceInterpreter != nil),
      ("MobileFaceNet (Recognition)", "mobilefacenet", faceNetInterpreter != nil),
      ("MediaPipe FaceMesh (Liveness)", "facemesh", false)
    ]

    let result = models.map { name, resource, loaded -> [String: Any] in
      let path = Bundle.main.path(forResource: resource, ofType: "tflite")
      let size = path.flatMap { try? FileManager.default.attributesOfItem(atPath: $0)[.size] as? NSNumber }?.doubleValue ?? 0
      return [
        "name": name,
        "filename": "\(resource).tflite",
        "sizeBytes": size,
