...
export class AuthLogService {
  ...
  static async getPendingLogs(): Promise<AuthLog[]> {
    try {
      ...
      console.error('[AuthLogService] Fetching pending logs failed:', error);
      return [];
    }
  }
  ...
  static async getPendingCount(): Promise<number> {
    try {
      ...
      console.error('[AuthLogService] Getting pending count failed:', error);
      return 0;
    }
  }
  ...
  static async isRateLimited(now: number = Date.now()): Promise<boolean> {
    try {
      ...
      console.error('[AuthLogService] Rate limit check failed:', error);
      return false;
    }
  }
  ...
  static async getConfidenceTrend(userId: string): Promise<ConfidenceTrend | null> {
    try {
      ...
      console.error('[AuthLogService] Confidence trend calculation failed:', error);
      return null;
    }
  }
  ...
  static async detectAnomalies(log: Omit<AuthLog, 'id' | 'synced'>): Promise<AuthAnomaly[]> {
    ...
    try {
      ...
      console.error('[AuthLogService] Anomaly detection failed:', error);
    }
    return anomalies;
  }
}
