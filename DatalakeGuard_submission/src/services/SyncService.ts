...
export class SyncService {
  ...
  static async getStatus(): Promise<SyncStatus> {
    if (!this.lastSyncTime) {
      try {
        ...
        const [result] = await db.executeSql('SELECT last_sync_time FROM sync_meta WHERE id = 1', []);
        if (result.rows.length > 0) {
          this.lastSyncTime = result.rows.item(0).last_sync_time;
        }
      } catch (error) {
        console.error('[SyncService] Loading last sync time failed:', error);
      }
    }
  ...
  private static async saveLastSyncTime(time: number, deviceId: string): Promise<void> {
    try {
      ...
      console.error('[SyncService] Saving last sync time failed:', error);
    }
  }
}
