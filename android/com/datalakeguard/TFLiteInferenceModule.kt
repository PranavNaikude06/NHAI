...
class TFLiteInferenceModule(private val reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    private fun loadModels() {
        ...
        Log.d("TFLiteInference", "All models loaded successfully (BlazeFace, MobileFaceNet, FaceMesh).")
        ...
        faceMeshInterpreter?.let { interpreter ->
            val inputShape = interpreter.getInputTensor(0).shape()
            Log.e("TFLiteInference", "FaceMesh - Input Shape: ${inputShape.contentToString()}, Input Bytes: ${interpreter.getInputTensor(0).numBytes()}")
        }
    }
    ...
    @ReactMethod
    fun getModelDiagnostics(promise: Promise) {
        try {
            val models = listOf(
                Triple("BlazeFace (Detection)", "blazeface.tflite", blazeFaceInterpreter != null),
                Triple("MobileFaceNet (Recognition)", "mobilefacenet.tflite", faceNetInterpreter != null),
                Triple("MediaPipe FaceMesh (Liveness)", "facemesh.tflite", faceMeshInterpreter != null)
            )

            val result = WritableNativeArray()
            for ((name, filename, loaded) in models) {
                val descriptor = reactContext.assets.openFd("models/$filename")
                val item = Arguments.createMap().apply {
                    putString("name", name)
                    putString("filename", filename)
                    putDouble("sizeBytes", descriptor.length.toDouble())
                    putString("status", if (loaded) "loaded" else "missing")
                }
                descriptor.close()
                result.pushMap(item)
            }
            ...
            promise.reject("MODEL_DIAGNOSTICS_ERROR", "Could not read model diagnostics: ${e.message}", e)
        }
    }
    ...
    fun runFullPipeline(
        ...
        try {
            ...
            val laplacian = computeLaplacianVarianceNative(
                rawFloats,
                srcWidth,
                srcHeight,
                box[0],
    ...
    /**
     * Decode a JPEG file from a file:// URI or absolute path, extract its RGB
     * pixels natively with BitmapFactory, then run the full
     * BlazeFace → FaceNet → VectorSearch pipeline.
     *
     * This is the correct entry-point when the photo comes from
     * VisionCamera's capturePhoto() — the photo is always written as a JPEG
     * file on Android, so passing raw pixel arrays across the JS bridge is
     * unnecessary and produces the "unsupported pixel buffer layout" error.
     */
    @ReactMethod
    fun runFullPipelineFromFile(
        fileUri: String,
        enableCLAHE: Boolean,
        clipLimit: Double,
        tileSize: Double,
        ...
        try {
            // 1. Resolve file path (strip file:// scheme if present)
            ...
            val file = File(path)
            if (!file.exists()) {
                promise.reject("FILE_ERROR", "Photo file not found: $path")
                return
            }

            // 2. Decode JPEG → Bitmap → ARGB_8888
            val rawBitmap = BitmapFactory.decodeFile(path)
                ?: run {
                    promise.reject("DECODE_ERROR", "BitmapFactory could not decode: $path")
                    return
                }
            val bitmap: Bitmap = if (rawBitmap.config == Bitmap.Config.ARGB_8888) {
                rawBitmap
            } else {
            ...
            val argbInts  = IntArray(pixelCount)
            bitmap.getPixels(argbInts, 0, srcWidth, 0, 0, srcWidth, srcHeight)
            bitmap.recycle()

            // 3. Convert ARGB ints → RGB FloatArray
            val rawFloats = FloatArray(pixelCount * 3)
            for (i in 0 until pixelCount) {
                val argb = argbInts[i]
                rawFloats[i * 3]     = ((argb shr 16) and 0xFF).toFloat() // R
                rawFloats[i * 3 + 1] = ((argb shr  8) and 0xFF).toFloat() // G
                rawFloats[i * 3 + 2] = ( argb          and 0xFF).toFloat() // B
            }

            // 4. BlazeFace (128×128)
            val blazeInput = resizeRGBNative(rawFloats, srcWidth, srcHeight, 128, 128)
            val blazeBuf = ByteBuffer.allocateDirect(4 * 128 * 128 * 3).apply { order(ByteOrder.nativeOrder()) }
            for (i in 0 until 128 * 128 * 3) { blazeBuf.putFloat(blazeInput[i] / 255.0f) }
            blazeBuf.rewind()

            val outRegs   = Array(1) { Array(896) { FloatArray(16) } }
            val outScores = Array(1) { Array(896) { FloatArray(1)  } }
            val outputs   = HashMap<Int, Any>()
            outputs[0]    = outRegs
            outputs[1]    = outScores
            blazeFace.runForMultipleInputsOutputs(arrayOf(blazeBuf), outputs)

            var maxScore = -1.0f
            var bestIdx  = -1
            for (i in 0 until 896) {
                val s = outScores[0][i][0]
            ...
            val box = outRegs[0][bestIdx]
            val boxArray = WritableNativeArray().apply {
                pushDouble(box[0].toDouble()); pushDouble(box[1].toDouble())
                pushDouble(box[2].toDouble()); pushDouble(box[3].toDouble())
                pushDouble(maxScore.toDouble())
            }

            // 5. FaceNet crop (112×112) + optional CLAHE
            val cropped = cropAndResizeRGBNative(
                rawFloats, srcWidth, srcHeight,
                box[0], box[1], box[2], box[3],
                112, 112
            )
            val processed = if (enableCLAHE) {
                applyCLAHE(cropped, 112, 112, clipLimit.toFloat(), tileSize.toInt(), tileSize.toInt())
            } else {
                cropped
            }

            val fnBuf = ByteBuffer.allocateDirect(2 * 4 * 112 * 112 * 3).apply { order(ByteOrder.nativeOrder()) }
            for (b in 0 until 2) {
                for (i in 0 until 112 * 112 * 3) { fnBuf.putFloat((processed[i] - 127.5f) / 128.0f) }
            }
            fnBuf.rewind()

            val fnOut = ByteBuffer.allocateDirect(2 * 192 * 4).apply { order(ByteOrder.nativeOrder()) }
            faceNet.run(fnBuf, fnOut)
            fnOut.rewind()

            val embedding = FloatArray(192) { fnOut.getFloat() }

            // 6. Vector search
            val matchResult = VectorSearchEngine.findBestMatch(embedding)

            // 7. BlazeFace keypoints as landmarks
            val landmarksArray = WritableNativeArray()
            for (k in 0 until 6) {
                landmarksArray.pushDouble(box[4 + k * 2 + 1].toDouble()) // x
                landmarksArray.pushDouble(box[4 + k * 2].toDouble())     // y
                landmarksArray.pushDouble(0.0)                            // z
            }
            ...
            Log.e("TFLiteInference", "runFullPipelineFromFile error: ${e.message}", e)
            promise.reject("INFERENCE_ERROR", "Pipeline from file failed: ${e.message}", e)
        }
    }

    private fun computeLaplacianVarianceNative(
        pixels: FloatArray,
        srcWidth: Int,
        srcHeight: Int,
        x: Float,
        ...
        val scaleX = cropW.toFloat() / targetW
        val scaleY = cropH.toFloat() / targetH

        for (cy in 0 until targetH) {
            val srcY = Math.min(srcHeight - 1, yStart + (cy * scaleY).toInt())
            val rowOffset = srcY * srcWidth
            for (cx in 0 until targetW) {
                val srcX = Math.min(srcWidth - 1, xStart + (cx * scaleX).toInt())
                val srcIdx = (rowOffset + srcX) * 3

                val r = pixels[srcIdx]
                val g = pixels[srcIdx + 1]
                val b = pixels[srcIdx + 2]
                gray[cy * targetW + cx] = Math.round(0.299f * r + 0.587f * g + 0.114f * b)
            }
        }
        ...
        for (cy in 1 until targetH - 1) {
            for (cx in 1 until targetW - 1) {
                val lap = (
                    gray[(cy - 1) * targetW + cx] +
                    gray[(cy + 1) * targetW + cx] +
                    gray[cy * targetW + (cx - 1)] +
                    gray[cy * targetW + (cx + 1)] -
                    4 * gray[cy * targetW + cx]
                )
                sum += lap.toDouble()
    ...
    @ReactMethod
    fun decodeJpegToRgb(fileUri: String, maxSize: Int, promise: Promise) {
        try {
            val path = if (fileUri.startsWith("file://")) {
                URI(fileUri).path
            } else {
            ...
            val scale = Math.min(1.0f, maxSize.toFloat() / Math.max(srcWidth, srcHeight))
            val targetWidth  = Math.max(1, Math.round(srcWidth * scale))
            val targetHeight = Math.max(1, Math.round(srcHeight * scale))

            val bitmap = Bitmap.createScaledBitmap(rawBitmap, targetWidth, targetHeight, true)
            if (bitmap != rawBitmap) {
                rawBitmap.recycle()
            }
            ...
            val argbInts  = IntArray(pixelCount)
            bitmap.getPixels(argbInts, 0, targetWidth, 0, 0, targetWidth, targetHeight)
            bitmap.recycle()

            val rgbArray = WritableNativeArray()
            for (i in 0 until pixelCount) {
                val argb = argbInts[i]
                rgbArray.pushInt((argb shr 16) and 0xFF) // R
                rgbArray.pushInt((argb shr  8) and 0xFF) // G
                rgbArray.pushInt( argb          and 0xFF) // B
            }

            val result = Arguments.createMap().apply {
                putInt("width", targetWidth)
                putInt("height", targetHeight)
                putArray("pixels", rgbArray)
            }
            promise.resolve(result)
        } catch (e: Exception) {
            Log.e("TFLiteInference", "decodeJpegToRgb error: ${e.message}", e)
            promise.reject("DECODE_ERROR", "Decoding failed: ${e.message}", e)
        }
    }
}
