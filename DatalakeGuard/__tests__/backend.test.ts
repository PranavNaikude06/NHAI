...
describe('Backend Services Integration Tests', () => {
  ...
  describe('1. EncryptionService', () => {
    ...
    test('should encrypt and decrypt embeddings float arrays correctly', async () => {
      ...
      const originalEmbedding = Array.from({ length: 128 }, (_, i) => i * 0.01);
  ...
  describe('3. EmbeddingService', () => {
    ...
    test('should retrieve and decrypt embeddings from DB', async () => {
      ...
      const mockEncryptedVal = await EncryptionService.encryptEmbedding(Array.from({ length: 128 }, () => 0.1));
      ...
      expect(results[0].embedding).toEqual(Array.from({ length: 128 }, () => 0.1));
      expect(results[0].vector).toEqual(Array.from({ length: 128 }, () => 0.1));
    });
  });
